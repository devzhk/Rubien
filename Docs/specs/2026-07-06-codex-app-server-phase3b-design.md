...
**Date:** 2026-07-06
**Status:** Historical Phase-3b design. The provider shipped, but its original
per-window topology and launch-time Apps/Rubien MCP wiring were superseded by
the shared-broker and partial B-T work. See
`2026-07-23-assistant-runtime-architecture-options.md` for the current
architecture and `2026-07-23-codex-bt-spike-results.md` for the 0.142.5/0.145
behavioral evidence. Protocol details below remain useful; architecture and
configuration passages are retained as historical rationale unless explicitly
marked current.
...
- **Rubien MCP channel — current implementation:** new and cold-resumed
  threads carry a complete canonical `mcp_servers.rubien` entry in `config`.
  The field-complete entry replaces a user-configured `rubien` entry for that
  thread and selects `rubien-cli mcp` or `rubien-cli mcp --read-only`.
- **Drop codex's built-in connectors — current implementation:** the thread
  config pins `features.apps=false` unless the interactive conversation opts
  in. Installed plugins remain a process flag because runtime isolation has
  not been proven.
- **Env:** minimal allowlist (`HOME,USER,LANG,LC_ALL,TMPDIR`, `TERM=dumb`, `NO_COLOR=1`) + `PATH` (binary dir plus `/usr/local/bin`, `/opt/homebrew/bin`, `/usr/bin`, `/bin`); keep `HOME` so `~/.codex` resolves; never inherit the app env (§4.1 master), even when connected tools are enabled. Provider-configured credentials work, while shell-only variables and custom `PATH` entries do not. **No `CODEX_HOME`.**
...
- **The one NET-NEW exposure vs Claude is cloud-CONNECTOR reads** (mail/chat/docs) that Claude's strict config can't reach. Codex's built-in connector surface (`codex_apps`) is **dropped by thread-scoped `features.apps=false`**, and installed plugins are separately dropped by default with `--disable plugins`. What remains is any *user-configured cloud-connector* MCP server in `~/.codex` — which this user does not have (only `node_repl`, local).
- **Ask vs Auto (the modes):** in the default **Ask mode**, calls Codex classifies as side-effecting prompt (Rubien cards any non-rubien tool it is asked to approve — `isSilentReadTool` allowlists only `mcp__rubien__*` + builtins); a read-only or incorrectly annotated tool may run silently. In **Auto mode** (`autoApprove`, opt-in) Rubien accepts every request it does receive.
- **Net:** for a `~/.codex` of local tooling, **no managed home + Apps/plugins disabled ≈ Claude's accepted posture**. The residual (a read-only *cloud-connector* tool running silently in Ask mode) only materializes if the user adds a cloud-connector MCP server to `~/.codex`; at that point a **"Strict Codex"** mode (a minimal managed home so only rubien loads) is the fix, surfaced as a Settings note (§ 3b-3). Risk #16 downgraded (rubien de-dup via the `-c` key override); Risk #8 (effort stall) handled per-request.

**Relationship to the "Use my other MCP servers" opt-in (master §5.5; implemented 2026-07-15, Codex plugin policy updated 2026-07-23):** The shipped label is **Use connected apps, plugins, and MCP tools**. The toggle swings exactly one thing — the **tool environment** — between two postures:
- **OFF (default) = isolate where the provider permits it.** Claude loads only Rubien via `--setting-sources '' --strict-mcp-config`. Codex disables built-in Apps and installed plugins and injects Rubien, but manually configured `~/.codex` MCP servers remain because app-server has no strict-config analogue; a paired **Strict Codex** managed home would be required for exact parity.
- **ON = wrap the user's configured agent.** Claude's MCP servers/plugins/settings and Codex's Apps/plugins load, and Rubien adds its **context**: it is their agent, now aware of the paper. Positive use case: *"summarize this paper and save it to my Notion."* A connector write shows a Rubien card only when the provider emits an approval request; existing permissions or tool annotations may allow it without one. Codex pins Apps on in thread config and plugins on at process launch, overriding user/project feature settings; Claude omits both isolation flags while retaining the explicit Rubien config. The canonical `rubien` key is replaced, but aliased Rubien servers are an accepted residual. The configured tools still run under Rubien's minimal child environment, not the launching shell's full environment.
- **Constant in BOTH postures:** (a) the **Rubien context** — the injected Rubien MCP server + the one-line reference seed; (b) the **transport + approval plumbing** — stream-json / JSON-RPC plus `--permission-prompt-tool stdio` / Codex `approvalPolicy:"on-request"`. The transport lets Rubien answer requests the provider emits; it does not override Claude permissions loaded by the opt-in or repair incorrect third-party tool annotations.
- **Current wiring:** `loadUserTools` selects thread-scoped Apps and
  process-scoped plugins. Both directions are pinned, so user/project feature
  configuration cannot contradict the Rubien toggle.

## 5. Architecture

```
CodexProvider : AgentProvider  (final class, Sendable)
 └── shared CodexRuntimeBroker (actor)    — app-lifetime admission + server state
 ...
 └── CodexInvocation (pure)               — process argv + per-thread config + minimal env;
                                            unit-tested; mirrors ClaudeCLIInvocation. NO managed CODEX_HOME.
```
- **Current topology:** provider wrappers share one registry-owned broker and
  one compatible app-server generation, with up to four concurrent turns.
  Window close releases that wrapper's thread lease; app shutdown owns final
  server teardown. The original one-server-per-window paragraph is
  superseded.
...
- **`AgentProvider` — one default-implemented addition:** `shutdown()` (default `{ cancel() }`), because `teardown` (window close) and `stop` must diverge for a long-lived process — Codex kills the server ONLY on `shutdown`; `cancel()` = `turn/interrupt` (server lives). Claude inherits the default (its per-turn process makes them identical), so zero behavior change. Otherwise unchanged: `send(turn:)` returns a per-turn stream (the connection tees the active turn's events into it); `respondToApproval(id:_:)` writes a `{decision}` response echoing the server id; `recentSessions/searchSessions/sessionTranscript` = `thread/list`/`thread/search`/`thread/read` (3b-4).
...
- **3b-2 — Invocation + connection + provider + fake server.** Extract `SpawnedAgentProcess` (shared). `CodexInvocation.swift` (pure argv + minimal-env builder: `app-server` with explicit process-scoped `--enable plugins` / `--disable plugins`; thread config carries Apps plus the canonical Rubien MCP entry selected by `loadUserTools`; NO managed `CODEX_HOME` — §4). `CodexAppServerConnection` (actor: spawn+handshake, request/response correlation, server-request routing, per-turn notification routing, lifecycle). `CodexProvider: AgentProvider`. A committed **fake app-server** test executable (emits controlled JSON-RPC incl. an approval request, floods, delays, exits non-zero) → drives handshake, turn routing, approval round-trip, `turn/interrupt`, process-group kill (no orphan), unknown-method tolerance — mirrors the fake-claude harness.
- **3b-3 — Provider picker + wiring + Settings.** `ReaderChatSession.make` selects the provider from a new `RubienPreferences.assistantProvider`; composer-header **provider picker** (Claude ▾ / Codex ▾ — switch = fresh conversation on the other runtime, rebuilding the controller). `AssistantModelOptions` codex model list. Settings ▸ Assistant: codex model override, reasoning effort (default **medium**), default sandbox (`read-only`) + `workspace-write` opt-in, web default, codex auth status + Recheck, disclosure of codex's session store (real `~/.codex`). **Note the MCP posture (§4):** Codex loads your own configured `~/.codex` MCP servers even while the default posture drops built-in Apps and installed plugins; a Settings disclosure explains this + names any loaded non-rubien servers, with a **"Strict Codex" toggle (default off, follow-up)** that would isolate to rubien-only for users with cloud-connector servers.
...
| C3 | No `--strict-mcp-config` analogue → the user's own configured codex MCP servers load alongside injected rubien; a read-only *cloud-connector* tool could be invoked silently by a hostile doc | **Scoped & accepted** (user 2026-07-06, §4): silent local reads are already accepted for both providers; built-in connectors and installed plugins are dropped by default; net-new risk = user-configured cloud connectors. Provider-classified mutations can raise a Rubien card, but user rules and inaccurate annotations can bypass it. A managed Codex home would be required for strict Rubien-only mode. |
...
- No `AgentEvent`/`ApprovalDecision` changes; `AgentProvider` gains only the default-implemented `shutdown()` (§5 — Claude inherits the default). `ChatSessionController` changes one line (`teardown()` calls `shutdown()` instead of `cancel()`); no other controller logic changes (the composition root picks the provider). No renderer changes. No library writes (still Phase 4 — Codex mutations in v1 are the agent operating on the *workspace folder*, gated by the approval card; the MCP content channel stays **read-only**). Rubien still persists nothing (D5) — codex owns its session store in the real `~/.codex` (no managed home; History scopes by `cwd` — §2.3).
