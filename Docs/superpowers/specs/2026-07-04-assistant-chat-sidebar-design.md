...
- **Phase 2b-ii — Wire the content channel into the Claude provider.** At the reserved extension point in `ClaudeCLIInvocation.arguments`, provide an ephemeral `--mcp-config` (naming the already-bundled `Contents/Helpers/rubien-cli` with `args:["mcp","--read-only"]` and `env:{RUBIEN_LIBRARY_ROOT:<the app's resolved library root>}`) plus `--strict-mcp-config` so only Rubien's server loads; resolve the bundled `rubien-cli` path; `isAvailable()` gains a fast MCP health check. Unit-tested via the invocation/config builders (`RubienTests`); no UI yet (that's 2c). *(The provider is constructed by the app only in Phase 2c, so this wiring is exercised live there.)*
