...
- **v1 (Phase 2b): read-only server mode** — `rubien-cli mcp --read-only` registers only the six read tools (`pdf_text`/`pdf_page_image`/`get`/`annotations`/`web_get`/`search`), its **tool contract mirroring the npm server exactly** (names/input-schemas/output-shapes — `mcp-server/src/tools/*.ts` is the reference) so the two are drop-in interchangeable. Since MCP bypasses the codex sandbox, **the approval channel — not `-s` — is what would gate MCP writes**; in read-only that is moot (no write tools registered).
...
- **MCP content channel (Phase 2b):** `rubien-cli mcp --read-only` speaks JSON-RPC 2.0 (initialize / tools/list / tools/call) exposing exactly the six read tools, with names/schemas/outputs matching the npm server (a **parity check** against `mcp-server/src/tools/*.ts`); `isAvailable()`'s MCP health probe passes when the server answers `initialize`. In `RubienCLITests` (stdio/JSON contract) + a Swift wiring assertion that the Claude argv injects `--mcp-config … --strict-mcp-config` at the reserved extension point. **No Node.**
...
- **Phase 2b — Native MCP content channel.** Add the MCP-over-stdio mode to `rubien-cli` (`mcp` subcommand, `--read-only`): JSON-RPC 2.0 (initialize / tools/list / tools/call) registering only the six read tools, its **tool contract mirroring the npm `rubien-mcp-server` exactly** (names/input-schemas/output-shapes per `mcp-server/src/tools/*.ts`) so the two are drop-in interchangeable. Wire it into the Claude provider at the reserved extension point — the app writes an ephemeral `--mcp-config` JSON naming the **already-bundled** `Contents/Helpers/rubien-cli` with `args:["mcp","--read-only"]` (+ `RUBIEN_LIBRARY_ROOT`), plus `--strict-mcp-config` so only Rubien's server loads; `isAvailable()` gains a fast MCP health probe. **No Node/runtime gate; nothing new bundled.** Update `Docs/CLI-Reference.md` (CLI lockstep).
