...
**Relationship to the "Use my other MCP servers" opt-in (master §5.5; implemented 2026-07-15):** The shipped label is **Use connected apps and MCP tools**. The toggle swings exactly one thing — the **tool environment** — between two postures:
- **OFF (default) = isolate where the provider permits it.** Claude loads only Rubien via `--setting-sources '' --strict-mcp-config`. Codex disables built-in Apps and injects Rubien, but manually configured `~/.codex` MCP servers remain because app-server has no strict-config analogue; a paired **Strict Codex** managed home would be required for exact parity.
- **ON = wrap the user's configured agent.** Claude's MCP servers/plugins/settings and Codex's Apps load, and Rubien adds its **context**: it is their agent, now aware of the paper. Positive use case: *"summarize this paper and save it to my Notion."* A connector write shows a Rubien card only when the provider emits an approval request; existing permissions or tool annotations may allow it without one. Codex omits `--disable apps`; Claude omits both isolation flags while retaining the explicit Rubien config. The canonical `rubien` key is replaced, but aliased Rubien servers are an accepted residual. The configured tools still run under Rubien's minimal child environment, not the launching shell's full environment.
