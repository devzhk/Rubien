...
- **Phase C (C3) — collapsed under Mac-only-first.** Phase C's structural deliverables (reader gating on cached PDFs, annotation count decoupled from PDF presence at the data layer, failable `PDFReaderView` convenience init) all shipped as part of Task 8's broadened scope on Phase A. The remaining nominal items don't apply here: **Task 17 (`PDFAvailability` view modifier) deferred to the iOS-port plan** (its load-bearing value is the `.downloading` state, which only exists when on-demand fetch lands; chip rendering already gates on `Reference.hasPDFInCache(in:)` directly without a state-machine wrapper). **Task 18 (`SyncedLibrary.fetchAsset(refId:)`) deferred** to the iOS-port plan as already noted. **Task 19 (reader integration)** done via Task 8's failable `PDFReaderView` convenience init. **Task 20 (annotation count decoupling)** done via Task 8 — fetch is independent of PDF presence; the Mac UI keeps showing the count inline with the chip because annotations always coexist with the PDF on Mac (cases where they wouldn't are iOS-only). **Task 21 (Phase C checkpoint)** subsumed into Phase B's Task 16 checkpoint since no new code lands here.
