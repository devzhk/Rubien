...
`Sources/RubienCLI/RubienCLI.swift` is the single-file argument-parser entry with 15 subcommands (`search`, `list`, `get`, `add`, `update`, `delete`, `cite`, `import`, `export`, `properties`, `views`, `annotations`, `styles`, `pdf`, `sync`). Tag operations are exposed through `properties` against the seeded built-in Tags PropertyDefinition (`defaultFieldKey == "tags"`) — see `Docs/CLI-Reference.md` for the migration mapping from the retired `tags` subcommand. JSON is the default output format — scripts depend on it, so don't change CLI output shape without updating tests in `Tests/RubienCLITests/`.
...
- **Type vs Status: built-in property mutability.** Both Type (`referenceType`) and Status (`readingStatus`) are seeded as `isDefault=true` PropertyDefinitions, but they have *different* mutability rules — encoded in the `Properties.optionsMutable(for:)` predicate in `Sources/RubienCLI/RubienCLI.swift`:
  - **Type is fixed.** Its 6 cases (Journal Article / Conference Paper / Book / Thesis / Web Page / Other) map 1:1 to BibTeX entry types and drive the `cslType`, BibTeX, and RIS export switches. Adding a "Lecture Notes" Type would just emit `@misc` — no semantic distinction. Webpage stays as a sixth case despite collapsing to `@misc` because it gates the in-app web reader (`Reference.canOpenWebReader`). The CLI/UI explicitly point users at Tags or `properties --create` for organization needs.
  - **Status is user-extensible.** It has zero coupling to BibTeX/CSL output, so the option set is whatever the user has configured. Add/rename/delete go through `AppDatabase.renamePropertyOption(propertyId:from:to:)` and `deletePropertyOption(propertyId:value:replaceWith:)`, which update `optionsJSON` AND bulk-update affected `reference.readingStatus` rows in one transaction. The seeded 4 (Unread / Reading / Skimmed / Read) can also be renamed or deleted.
  - When adding a new built-in property in the future, decide which bucket it belongs in — and wire `optionsMutable` accordingly. Custom (non-default) properties always allow option mutations.
