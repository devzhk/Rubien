...
export function registerReferenceTools(server: McpServer): void {
  server.registerTool(
    ...
    async ({ query, limit, in: inFields, scope, op }) => {
      if (scope && inFields?.length) {
        return {
          isError: true,
          content: [{ type: "text", text: "Choose scope or in, not both" }],
        };
      }
      return runCliAsTool([
        "search",
