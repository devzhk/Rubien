...
final class PaperMentionsTests: XCTestCase {
    ...
    func testSelectionsStillPresentLetsValidDuplicateSurviveStaleEarlierRange() {
        let bert = ChatReference(id: 8, title: "BERT", authors: "Devlin et al.")
        let text = "@BERT"
        let stale = PaperMentionSelection(reference: bert, range: 1..<6)
        let valid = PaperMentionSelection(reference: bert, range: 0..<5)

        XCTAssertEqual(
            PaperMentions.selectionsStillPresent(in: text, from: [stale, valid]),
            [valid]
        )
    }
