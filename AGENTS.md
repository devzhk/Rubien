...
- **Cut a release:** see `Docs/Release-Runbook.md`; its order is mandatory. Short version: start from a clean/current `main`; bump `VERSION` as needed and `BUILD.txt` every time; regenerate `Sources/RubienCLI/GeneratedVersion.swift`; update any coupled package version (for example `mcp-server`) before the pre-release commit; draft fresh release notes; commit all release prep; push `main`; and watch the CI run for that **exact `HEAD` SHA** to `success`. If the push is docs-only and CI is skipped by `paths-ignore`, dispatch `ci.yml` manually for `main` and verify that exact SHA. Only then run `./scripts/release.sh` on the interactive host, **not** from a sandboxed agent (e.g. Codex). A green run for an older commit or local tests is not sufficient, and `release.sh`'s later appcast push is not the pre-release validation push.
- **After `release.sh`:** verify the public DMG release, production appcast item, and private source tag; watch the dispatched Linux CLI workflow to success; confirm the DMG, Linux tarball, and tarball signature are all attached; and copy the printed dSYM archive to durable private storage. Only after the matching CLI is live should a coupled npm package be published.
...
- **Pass multi-line `RELEASE_NOTES_TEXT` as an ANSI-C `$'…'` string, NOT a pasted heredoc.** `RELEASE_NOTES_TEXT="$(cat <<'NOTES' … NOTES)"` hangs the shell on copy-paste: the terminal indents the pasted lines, but a `<<'NOTES'` terminator must sit at column 0, so the heredoc never closes (you land at `dquote cmdsubst heredoc>`; `Ctrl-C` escapes). Use `RELEASE_NOTES_TEXT=$'• line one\n• line two' ./scripts/release.sh` instead — newlines as `\n`, one logical line, paste-proof — or a single-line `"…"` string.
- **The MCP npm package has its own version and publication gate.** Before the pre-release commit, compare `mcp-server/package.json` with `npm view rubien-mcp-server version`; if that version is already published and the package changed, bump `package.json`, both root-version entries in `package-lock.json`, and `SERVER_INFO.version` together, then run its build/tests before pushing. After the app release, wait for the matching Linux CLI asset and verify its build satisfies `mcp-server/src/versionGuard.ts` `MIN_CLI_BUILD`; only then run `npm publish` and verify the registry reports the intended new version.
...
- **Mac releases are Apple Silicon-only, coverage-free, stripped, and symbolicated.** `scripts/build-app.sh` pins Release to exactly `arm64` with coverage off, thins the binary-target Sparkle framework to arm64, and rejects any non-arm64 Mach-O anywhere in the assembled app. It also rejects `__LLVM_COV` / `__llvm_prf_*`, UUID-verifies and preserves the app + CLI dSYMs in a UUID-keyed `build/dSYMs/*.dSYMs.zip`, then runs `strip -S -x` after all binary mutations and before codesigning. Keep that order: thinning or stripping after signing invalidates the signature, while relying on Xcode scheme state can silently change the architecture or add several megabytes of coverage data. Never overwrite a different UUID archive for the same version/build. This does not change the separately-built Linux CLI architecture policy.
- **DMG size hygiene:** the mounted-volume icon is intentional and stays embedded as `.VolumeIcon.icns`; do not remove `--volicon` just to save space. Do not re-add `NSWorkspace.setIcon` for the downloaded `.dmg` file: it creates a large ResourceFork xattr that GitHub/HTTP uploads do not carry and makes local `du` output misleading. The release image uses APFS + ULFO/LZFSE, and downloadable size is the data-fork byte count (`stat -f '%z'`), not `du`. `release.sh` checks both the pre-notarization and final stapled images and rejects unexplained growth over 2 MiB unless `ALLOW_DMG_SIZE_GROWTH=1` explicitly acknowledges an audited increase.
